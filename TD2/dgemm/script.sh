...
do  
    ...
    do
        ...
        do 
            ...
            sed -n 2p ./$chemin/"$c$flag"_"$o".csv | cut -d";" -f1,11 | sed "s/^\ *//g"  >> ./gnuplot/"$c$flag"_output.csv

            #extraire la val Mib/s pour la comparaison
            sed -n 2p ./$chemin/"$c$flag"_"$o".csv | cut -d";" -f11 | sed "s/^\ *//g"  >> ./gnuplot/"$c$flag"_flag.csv
